...
func TestDetector_Detect_EOLVersion(t *testing.T) {
	// Setup mock inventory with EOL EKS cluster
	eolDate := time.Now().AddDate(0, -6, 0) // 6 months ago
	mockInventory := &inventorymock.InventorySource{
		Resources: []*types.Resource{
			{
				ID:             "arn:aws:eks:us-east-1:123456:cluster/test-cluster",
				Name:           "test-cluster",
	...
	if finding.ResourceID != "arn:aws:eks:us-east-1:123456:cluster/test-cluster" {
...
func TestDetector_Detect_CurrentVersion(t *testing.T) {
	// Setup mock inventory with current EKS cluster
	eolDate := time.Now().AddDate(2, 0, 0) // 2 years from now
	mockInventory := &inventorymock.InventorySource{
		Resources: []*types.Resource{
			{
				ID:             "arn:aws:eks:us-west-2:789012:cluster/prod-cluster",
				Name:           "prod-cluster",
				Type:           types.ResourceTypeEKS,
				CloudProvider:  types.CloudProviderAWS,
				Service:        "payments",
...
func TestDetector_Detect_ExtendedSupport(t *testing.T) {
	...
	mockInventory := &inventorymock.InventorySource{
		Resources: []*types.Resource{
			{
				ID:             "arn:aws:eks:us-east-1:123456:cluster/legacy-cluster",
				Name:           "legacy-cluster",
				Type:           types.ResourceTypeEKS,
				CloudProvider:  types.CloudProviderAWS,
				Service:        "legacy-app",
...
func TestDetector_Detect_MultipleResources(t *testing.T) {
	...
	mockInventory := &inventorymock.InventorySource{
		Resources: []*types.Resource{
			{
				ID:             "arn:aws:eks:us-east-1:123456:cluster/eol-cluster",
				Name:           "eol-cluster",
				Type:           types.ResourceTypeEKS,
				CloudProvider:  types.CloudProviderAWS,
				Service:        "old-app",
			...
			{
				ID:             "arn:aws:eks:us-east-1:123456:cluster/current-cluster",
	...
	for _, finding := range findings {
		if finding.Status == types.StatusRed {
			redFound = true
			if finding.ResourceID != "arn:aws:eks:us-east-1:123456:cluster/eol-cluster" {
				t.Errorf("Unexpected RED finding resource: %s", finding.ResourceID)
			}
		}
		if finding.Status == types.StatusGreen {
			greenFound = true
			if finding.ResourceID != "arn:aws:eks:us-east-1:123456:cluster/current-cluster" {
...
func TestDetector_ResourceType(t *testing.T) {
	...
	if resourceType != types.ResourceTypeEKS {
		t.Errorf("Expected resource type EKS, got %s", resourceType)
	}
}
