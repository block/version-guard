...
// Detector implements version drift detection for Aurora databases
...
func (d *Detector) Name() string {
	return "aurora-detector"
}

// ResourceType returns the resource type this detector handles
func (d *Detector) ResourceType() types.ResourceType {
	return types.ResourceTypeAurora
}

// Detect scans Aurora resources and detects version drift
func (d *Detector) Detect(ctx context.Context) ([]*types.Finding, error) {
	// Step 1: Fetch inventory
	resources, err := d.inventory.ListResources(ctx, types.ResourceTypeAurora)
	if err != nil {
		return nil, errors.Wrap(err, "failed to fetch Aurora inventory")
	}
