...
- If Version Guard generic infrastructure is not implemented, **STOP** and direct user to complete Phase 1 first
...
```bash
# 1. Verify generic infrastructure exists
test -f config/resources.yaml && echo "✅ Config schema exists" || echo "❌ Missing"
test -f pkg/config/loader.go && echo "✅ Config loader exists" || echo "❌ Missing"
test -f pkg/detector/generic/detector.go && echo "✅ Generic detector exists" || echo "❌ Missing"
test -f pkg/inventory/wiz/generic.go && echo "✅ Generic inventory exists" || echo "❌ Missing"
test -f pkg/eol/endoflife/adapters.go && echo "✅ EOL adapters exist" || echo "❌ Missing"
```

**STOP** if any prerequisite check fails.
...
2. **Wiz report ID** (the actual report UUID)
   ...
   - Example: `"7bac4838-cf54-46c4-93a2-f63cced1735a"`
...
```bash
# Look at existing config for reference
cat config/resources.yaml

# Examine test fixtures
grep -A 5 "CSVData" pkg/inventory/wiz/fixtures_test.go | head -20
```
...
- `examples/lambda.yaml` - Adding Lambda runtime detection (no version field, uses graphEntity.properties)
...
git commit -m "feat: add {resource-type} support
